...
// `deps` lets tests inject the model calls; defaults to the real harness.
export async function runCouncil(opts, deps = {}) {
  const { task, proposerSpec, verifierSpec, root = process.cwd(), allowShell, allowFetch, maxSteps, onEvent } = opts;
  ...
  const output = await _runAgent({
    spec: proposerSpec, task, root, allowShell, allowFetch, maxSteps,
  ...
  const res = await _chat({ spec: verifierSpec, messages, tools: [] });
